...
static const t_inst instructions[11] = 
{
	...
	{"pa\n", NULL, NULL, push},						// pa	3
	{"pb\n", NULL, NULL, push},						// pb	4
	{"ra\n", rotate, NULL, NULL},					// ra	5
	{"rb\n", NULL, rotate, NULL},					// rb	6
	{"rr\n", rotate, rotate, NULL},					// rr	7
	{"rra\n", reverse_rotate, NULL, NULL},			// rra	8
	{"rrb\n", NULL, reverse_rotate, NULL},			// rrb	9
	{"rrr\n", reverse_rotate, reverse_rotate, NULL}	// rrr	10
};
...
void sort_3_nums(t_list **lst)
{
	...
	{
		call_instruction(instructions[5], lst, NULL);
		if(!is_sorted(*lst))
			call_instruction(instructions[0], lst, NULL);
	}
	else if(values[0] < values[1] && values[0] < values[2])
	{
		call_instruction(instructions[8], lst, NULL);
		call_instruction(instructions[0], lst, NULL);
	}
	...
	while(*b)
	{
		...
		while(index > 1)
		{
			if(index <= ft_lstsize(*a) / 2)
				call_instruction(instructions[5], a, b);
			else
				call_instruction(instructions[8], a, b);
			index = get_index_in_list(find_target_position_in_a(*(int*)(*b)->content, a), a);
		}
		call_instruction(instructions[3], a, b);
	}
}
...
void push_best_in_b(t_list *best, t_list **a, t_list **b)
{
	...
	while(index_a > 1 && index_b > 1)
	{
		if(index_a <= ft_lstsize(*a) / 2 && index_b <= ft_lstsize(*b) / 2)
			call_instruction(instructions[7], a, b);
		else if(index_a > ft_lstsize(*a) / 2 && index_b > ft_lstsize(*b) / 2)
			call_instruction(instructions[10], a, b);
		else
			break ;
		index_a = get_index_in_list(best, a);
		index_b = get_index_in_list(find_target_position_in_b(*(int*)best->content, b), b);
	...
	while(index_b > 1)
	{
		if(index_b <= ft_lstsize(*b) / 2)
			call_instruction(instructions[6], a, b);
		else
			call_instruction(instructions[9], a, b);
		index_b = get_index_in_list(find_target_position_in_b(*(int*)best->content, b), b);
	}
	call_instruction(instructions[4], b, a);
}
...
int is_list_valid(t_list *lst)
{
	t_list *i;

	while(lst)
	{
		//DOESNT WORK TO CHECK INT MAX MIN, GO CHECK BEFORE ATOI
